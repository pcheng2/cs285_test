...
class FFModel(nn.Module, BaseModel):
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        """
        prediction, delta_pred_normalized = self(obs, acs, **data_statistics) # TODO(Q1) get numpy array of the predicted next-states (s_t+1)
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        target_var  = data_statistics['delta_std']
        # target = normalize((next_observations-observations),target_mean,target_var )# TODO(Q1) compute the normalized target for the model.
        
        target = normalize(next_observations,target_mean,target_var )
        # Hint: you should use `data_statistics['delta_mean']` and
        # `data_statistics['delta_std']`, which keep track of the mean
        # and standard deviation of the model.

        pred, delta_pred_normalized = self(observations, actions, **data_statistics)
